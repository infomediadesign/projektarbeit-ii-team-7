...
Quaternion quaternion_make(const f32 x, const f32 y, const f32 z, const f32 w) { return (Quaternion) { x, y, z, w }; }

Quaternion quaternion_from_vec(const Vector4 v) { return (Quaternion) { v.x, v.y, v.z, v.w }; }

Vector4 quaternion_to_vec(const Quaternion q) { return (Vector4) { q.x, q.y, q.z, q.w }; }

Quaternion quaternion_rotation(const Vector3 axis, const f32 rotation) {
  return (Quaternion) {
    axis.x * sinf(rotation * 0.5), axis.y * sinf(rotation * 0.5), axis.z * sinf(rotation * 0.5), cosf(rotation * 0.5)
  };
}
...
u8 quaternion_is_normal(const Quaternion *q) {
  return q->x >= -1.0f && q->x <= 1.0f && q->y >= -1.0f && q->y <= 1.0f && q->z >= -1.0f && q->z <= 1.0f &&
         q->w >= -1.0f && q->w <= 1.0f;
}
